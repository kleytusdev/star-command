...
<div x-data="{ show: false, name: '{{ $name }}' }" x-show="show" x-on:open-modal.window="show = ($event.detail.name === name)"
    ...
    class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center" x-transition.duration>
    {{-- Gray Background --}}
    <div x-on:click="show = false" x-transition:enter="transition ease-out duration-300 transform"
        ...
        class="fixed inset-0 transition-opacity bg-gray-300 dark:bg-dark-eval-0 opacity-40" aria-hidden="true">
    </div>
    ...
    <div x-transition:enter="transition ease-out duration-300 transform"
        ...
        class="inline-block w-full max-w-xl p-8 overflow-hidden text-left transition-all transform bg-white dark:bg-dark-eval-1 rounded-lg shadow-xl {{ $maxWidth }}">
        {{ $slot }}
    </div>
</div>
